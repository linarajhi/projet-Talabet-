...
import { UpdateParaa } from '../../redux/actions/parapharmacie';




const UpdatePara=({el,id}) =>{ 
  ...
  //updatePara
  const updatePARA=()=>{
      dispatch( UpdateParaa(id,update.nom,update.description,update.adresse,update.tel,update.image))
  ...
  setShow(false)
    ...
    <div>
        <>
      <Button className='button-admin me-5' variant="warning" onClick={handleShow}><ion-icon name="build"></ion-icon>
       modifier
      </Button>

      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Modifier Parapharmacie </Modal.Title>
        </Modal.Header>
        <Modal.Body>
        <Form>
  <Form.Group className="mb-3" controlId="formBasicEmail" >
    ...
    <Form.Control type="img" placeholder="Enter image" name='image' defaultValue={el.image}  onChange={handleChange}/>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={updatePARA}>
            Enregistrez
          </Button>
        </Modal.Footer>
      </Modal>
