...
import { addRestoApi } from '../../redux/actions/restaurants';
...
function AddResto() {
    ...
    //add restaurant
    const AddRestos=()=>{
        dispatch(addRestoApi(input.nom,input.description,input.adresse,input.tel,input.image))
        console.log("input",input)
        setShow(false)
    }
...
<Button variant="primary" onClick={handleShow}>
      <h5 className='titre'> Ajoutez Nouveau restaurant</h5>
      </Button>

      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Nouveau Restaurant</Modal.Title>
        </Modal.Header>
        <Modal.Body>
        <Form>
  <Form.Group className="mb-3" controlId="formBasicEmail" >
    ...
    <Form.Control type="img" placeholder="Enter image" name='image' onChange={handleChange} />
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={AddRestos}>
            Enregistrez
          </Button>
        </Modal.Footer>
